<?php
/**
 * ProductionLotApi
 * PHP version 5
 *
 * @category Class
 * @package  Infoplus
 * @author   http://github.com/swagger-api/swagger-codegen
 * @license  http://www.apache.org/licenses/LICENSE-2.0 Apache Licene v2
 * @link     https://github.com/swagger-api/swagger-codegen
 */
/**
 *  Copyright 2016 SmartBear Software
 *
 *  Licensed under the Apache License, Version 2.0 (the "License");
 *  you may not use this file except in compliance with the License.
 *  You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing, software
 *  distributed under the License is distributed on an "AS IS" BASIS,
 *  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the License for the specific language governing permissions and
 *  limitations under the License.
 */

/**
 * NOTE: This class is auto generated by the swagger code generator program. 
 * https://github.com/swagger-api/swagger-codegen 
 * Do not edit the class manually.
 */

namespace Infoplus\Api;

use \Infoplus\Configuration;
use \Infoplus\ApiClient;
use \Infoplus\ApiException;
use \Infoplus\ObjectSerializer;

/**
 * ProductionLotApi Class Doc Comment
 *
 * @category Class
 * @package  Infoplus
 * @author   http://github.com/swagger-api/swagger-codegen
 * @license  http://www.apache.org/licenses/LICENSE-2.0 Apache Licene v2
 * @link     https://github.com/swagger-api/swagger-codegen
 */
class ProductionLotApi
{

    /**
     * API Client
     * @var \Infoplus\ApiClient instance of the ApiClient
     */
    protected $apiClient;
  
    /**
     * Constructor
     * @param \Infoplus\ApiClient|null $apiClient The api client to use
     */
    function __construct($apiClient = null)
    {
        if ($apiClient == null) {
            $apiClient = new ApiClient();
            $apiClient->getConfig()->setHost('https://kingsrook.localhost-testsubdomain1.infopluswms.com:8443/infoplus-wms/api');
        }
  
        $this->apiClient = $apiClient;
    }
  
    /**
     * Get API client
     * @return \Infoplus\ApiClient get the API client
     */
    public function getApiClient()
    {
        return $this->apiClient;
    }
  
    /**
     * Set the API client
     * @param \Infoplus\ApiClient $apiClient set the API client
     * @return ProductionLotApi
     */
    public function setApiClient(ApiClient $apiClient)
    {
        $this->apiClient = $apiClient;
        return $this;
    }
  
    
    /**
     * addProductionLot
     *
     * Create a productionLot
     *
     * @param \Infoplus\Model\ProductionLot $body ProductionLot to be inserted. (required)
     * @return \Infoplus\Model\ProductionLot
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function addProductionLot($body)
    {
        list($response, $statusCode, $httpHeader) = $this->addProductionLotWithHttpInfo ($body);
        return $response; 
    }


    /**
     * addProductionLotWithHttpInfo
     *
     * Create a productionLot
     *
     * @param \Infoplus\Model\ProductionLot $body ProductionLot to be inserted. (required)
     * @return Array of \Infoplus\Model\ProductionLot, HTTP status code, HTTP response headers (array of strings)
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function addProductionLotWithHttpInfo($body)
    {
        
        // verify the required parameter 'body' is set
        if ($body === null) {
            throw new \InvalidArgumentException('Missing the required parameter $body when calling addProductionLot');
        }
  
        // parse inputs
        $resourcePath = "/beta/productionLot";
        $httpBody = '';
        $queryParams = array();
        $headerParams = array();
        $formParams = array();
        $_header_accept = ApiClient::selectHeaderAccept(array('application/json'));
        if (!is_null($_header_accept)) {
            $headerParams['Accept'] = $_header_accept;
        }
        $headerParams['Content-Type'] = ApiClient::selectHeaderContentType(array('application/json'));
  
        
        
        
        // default format to json
        $resourcePath = str_replace("{format}", "json", $resourcePath);

        
        // body params
        $_tempBody = null;
        if (isset($body)) {
            $_tempBody = $body;
        }
  
        // for model (json/xml)
        if (isset($_tempBody)) {
            $httpBody = $_tempBody; // $_tempBody is the method argument, if present
        } elseif (count($formParams) > 0) {
            $httpBody = $formParams; // for HTTP post (form)
        }
        
        // this endpoint requires API key authentication
        $apiKey = $this->apiClient->getApiKeyWithPrefix('API-Key');
        if (strlen($apiKey) !== 0) {
            $headerParams['API-Key'] = $apiKey;
        }
        
        
        // make the API Call
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                $resourcePath, 'POST',
                $queryParams, $httpBody,
                $headerParams, '\Infoplus\Model\ProductionLot'
            );
            
            if (!$response) {
                return array(null, $statusCode, $httpHeader);
            }

            return array(\Infoplus\ObjectSerializer::deserialize($response, '\Infoplus\Model\ProductionLot', $httpHeader), $statusCode, $httpHeader);
            
        } catch (ApiException $e) {
            switch ($e->getCode()) { 
            case 200:
                $data = \Infoplus\ObjectSerializer::deserialize($e->getResponseBody(), '\Infoplus\Model\ProductionLot', $e->getResponseHeaders());
                $e->setResponseObject($data);
                break;
            case 405:
                $data = \Infoplus\ObjectSerializer::deserialize($e->getResponseBody(), '\Infoplus\Model\ApiResponse', $e->getResponseHeaders());
                $e->setResponseObject($data);
                break;
            }
  
            throw $e;
        }
    }
    
    /**
     * addProductionLotAudit
     *
     * Add new audit for a productionLot
     *
     * @param int $production_lot_id Id of the productionLot to add an audit to (required)
     * @param string $production_lot_audit The audit to add (required)
     * @return void
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function addProductionLotAudit($production_lot_id, $production_lot_audit)
    {
        list($response, $statusCode, $httpHeader) = $this->addProductionLotAuditWithHttpInfo ($production_lot_id, $production_lot_audit);
        return $response; 
    }


    /**
     * addProductionLotAuditWithHttpInfo
     *
     * Add new audit for a productionLot
     *
     * @param int $production_lot_id Id of the productionLot to add an audit to (required)
     * @param string $production_lot_audit The audit to add (required)
     * @return Array of null, HTTP status code, HTTP response headers (array of strings)
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function addProductionLotAuditWithHttpInfo($production_lot_id, $production_lot_audit)
    {
        
        // verify the required parameter 'production_lot_id' is set
        if ($production_lot_id === null) {
            throw new \InvalidArgumentException('Missing the required parameter $production_lot_id when calling addProductionLotAudit');
        }
        // verify the required parameter 'production_lot_audit' is set
        if ($production_lot_audit === null) {
            throw new \InvalidArgumentException('Missing the required parameter $production_lot_audit when calling addProductionLotAudit');
        }
  
        // parse inputs
        $resourcePath = "/beta/productionLot/{productionLotId}/audit/{productionLotAudit}";
        $httpBody = '';
        $queryParams = array();
        $headerParams = array();
        $formParams = array();
        $_header_accept = ApiClient::selectHeaderAccept(array('application/json'));
        if (!is_null($_header_accept)) {
            $headerParams['Accept'] = $_header_accept;
        }
        $headerParams['Content-Type'] = ApiClient::selectHeaderContentType(array('application/json'));
  
        
        
        // path params
        
        if ($production_lot_id !== null) {
            $resourcePath = str_replace(
                "{" . "productionLotId" . "}",
                $this->apiClient->getSerializer()->toPathValue($production_lot_id),
                $resourcePath
            );
        }// path params
        
        if ($production_lot_audit !== null) {
            $resourcePath = str_replace(
                "{" . "productionLotAudit" . "}",
                $this->apiClient->getSerializer()->toPathValue($production_lot_audit),
                $resourcePath
            );
        }
        // default format to json
        $resourcePath = str_replace("{format}", "json", $resourcePath);

        
        
  
        // for model (json/xml)
        if (isset($_tempBody)) {
            $httpBody = $_tempBody; // $_tempBody is the method argument, if present
        } elseif (count($formParams) > 0) {
            $httpBody = $formParams; // for HTTP post (form)
        }
        
        // this endpoint requires API key authentication
        $apiKey = $this->apiClient->getApiKeyWithPrefix('API-Key');
        if (strlen($apiKey) !== 0) {
            $headerParams['API-Key'] = $apiKey;
        }
        
        
        // make the API Call
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                $resourcePath, 'PUT',
                $queryParams, $httpBody,
                $headerParams
            );
            
            return array(null, $statusCode, $httpHeader);
            
        } catch (ApiException $e) {
            switch ($e->getCode()) { 
            }
  
            throw $e;
        }
    }
    
    /**
     * addProductionLotTag
     *
     * Add new tags for a productionLot.
     *
     * @param int $production_lot_id Id of the productionLot to add a tag to (required)
     * @param string $production_lot_tag The tag to add (required)
     * @return void
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function addProductionLotTag($production_lot_id, $production_lot_tag)
    {
        list($response, $statusCode, $httpHeader) = $this->addProductionLotTagWithHttpInfo ($production_lot_id, $production_lot_tag);
        return $response; 
    }


    /**
     * addProductionLotTagWithHttpInfo
     *
     * Add new tags for a productionLot.
     *
     * @param int $production_lot_id Id of the productionLot to add a tag to (required)
     * @param string $production_lot_tag The tag to add (required)
     * @return Array of null, HTTP status code, HTTP response headers (array of strings)
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function addProductionLotTagWithHttpInfo($production_lot_id, $production_lot_tag)
    {
        
        // verify the required parameter 'production_lot_id' is set
        if ($production_lot_id === null) {
            throw new \InvalidArgumentException('Missing the required parameter $production_lot_id when calling addProductionLotTag');
        }
        // verify the required parameter 'production_lot_tag' is set
        if ($production_lot_tag === null) {
            throw new \InvalidArgumentException('Missing the required parameter $production_lot_tag when calling addProductionLotTag');
        }
  
        // parse inputs
        $resourcePath = "/beta/productionLot/{productionLotId}/tag/{productionLotTag}";
        $httpBody = '';
        $queryParams = array();
        $headerParams = array();
        $formParams = array();
        $_header_accept = ApiClient::selectHeaderAccept(array('application/json'));
        if (!is_null($_header_accept)) {
            $headerParams['Accept'] = $_header_accept;
        }
        $headerParams['Content-Type'] = ApiClient::selectHeaderContentType(array('application/json'));
  
        
        
        // path params
        
        if ($production_lot_id !== null) {
            $resourcePath = str_replace(
                "{" . "productionLotId" . "}",
                $this->apiClient->getSerializer()->toPathValue($production_lot_id),
                $resourcePath
            );
        }// path params
        
        if ($production_lot_tag !== null) {
            $resourcePath = str_replace(
                "{" . "productionLotTag" . "}",
                $this->apiClient->getSerializer()->toPathValue($production_lot_tag),
                $resourcePath
            );
        }
        // default format to json
        $resourcePath = str_replace("{format}", "json", $resourcePath);

        
        
  
        // for model (json/xml)
        if (isset($_tempBody)) {
            $httpBody = $_tempBody; // $_tempBody is the method argument, if present
        } elseif (count($formParams) > 0) {
            $httpBody = $formParams; // for HTTP post (form)
        }
        
        // this endpoint requires API key authentication
        $apiKey = $this->apiClient->getApiKeyWithPrefix('API-Key');
        if (strlen($apiKey) !== 0) {
            $headerParams['API-Key'] = $apiKey;
        }
        
        
        // make the API Call
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                $resourcePath, 'PUT',
                $queryParams, $httpBody,
                $headerParams
            );
            
            return array(null, $statusCode, $httpHeader);
            
        } catch (ApiException $e) {
            switch ($e->getCode()) { 
            }
  
            throw $e;
        }
    }
    
    /**
     * deleteProductionLot
     *
     * Delete a productionLot
     *
     * @param int $production_lot_id Id of the productionLot to be deleted. (required)
     * @return void
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function deleteProductionLot($production_lot_id)
    {
        list($response, $statusCode, $httpHeader) = $this->deleteProductionLotWithHttpInfo ($production_lot_id);
        return $response; 
    }


    /**
     * deleteProductionLotWithHttpInfo
     *
     * Delete a productionLot
     *
     * @param int $production_lot_id Id of the productionLot to be deleted. (required)
     * @return Array of null, HTTP status code, HTTP response headers (array of strings)
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function deleteProductionLotWithHttpInfo($production_lot_id)
    {
        
        // verify the required parameter 'production_lot_id' is set
        if ($production_lot_id === null) {
            throw new \InvalidArgumentException('Missing the required parameter $production_lot_id when calling deleteProductionLot');
        }
  
        // parse inputs
        $resourcePath = "/beta/productionLot/{productionLotId}";
        $httpBody = '';
        $queryParams = array();
        $headerParams = array();
        $formParams = array();
        $_header_accept = ApiClient::selectHeaderAccept(array('application/json'));
        if (!is_null($_header_accept)) {
            $headerParams['Accept'] = $_header_accept;
        }
        $headerParams['Content-Type'] = ApiClient::selectHeaderContentType(array());
  
        
        
        // path params
        
        if ($production_lot_id !== null) {
            $resourcePath = str_replace(
                "{" . "productionLotId" . "}",
                $this->apiClient->getSerializer()->toPathValue($production_lot_id),
                $resourcePath
            );
        }
        // default format to json
        $resourcePath = str_replace("{format}", "json", $resourcePath);

        
        
  
        // for model (json/xml)
        if (isset($_tempBody)) {
            $httpBody = $_tempBody; // $_tempBody is the method argument, if present
        } elseif (count($formParams) > 0) {
            $httpBody = $formParams; // for HTTP post (form)
        }
        
        // this endpoint requires API key authentication
        $apiKey = $this->apiClient->getApiKeyWithPrefix('API-Key');
        if (strlen($apiKey) !== 0) {
            $headerParams['API-Key'] = $apiKey;
        }
        
        
        // make the API Call
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                $resourcePath, 'DELETE',
                $queryParams, $httpBody,
                $headerParams
            );
            
            return array(null, $statusCode, $httpHeader);
            
        } catch (ApiException $e) {
            switch ($e->getCode()) { 
            }
  
            throw $e;
        }
    }
    
    /**
     * deleteProductionLotTag
     *
     * Delete a tag for a productionLot.
     *
     * @param int $production_lot_id Id of the productionLot to remove tag from (required)
     * @param string $production_lot_tag The tag to delete (required)
     * @return void
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function deleteProductionLotTag($production_lot_id, $production_lot_tag)
    {
        list($response, $statusCode, $httpHeader) = $this->deleteProductionLotTagWithHttpInfo ($production_lot_id, $production_lot_tag);
        return $response; 
    }


    /**
     * deleteProductionLotTagWithHttpInfo
     *
     * Delete a tag for a productionLot.
     *
     * @param int $production_lot_id Id of the productionLot to remove tag from (required)
     * @param string $production_lot_tag The tag to delete (required)
     * @return Array of null, HTTP status code, HTTP response headers (array of strings)
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function deleteProductionLotTagWithHttpInfo($production_lot_id, $production_lot_tag)
    {
        
        // verify the required parameter 'production_lot_id' is set
        if ($production_lot_id === null) {
            throw new \InvalidArgumentException('Missing the required parameter $production_lot_id when calling deleteProductionLotTag');
        }
        // verify the required parameter 'production_lot_tag' is set
        if ($production_lot_tag === null) {
            throw new \InvalidArgumentException('Missing the required parameter $production_lot_tag when calling deleteProductionLotTag');
        }
  
        // parse inputs
        $resourcePath = "/beta/productionLot/{productionLotId}/tag/{productionLotTag}";
        $httpBody = '';
        $queryParams = array();
        $headerParams = array();
        $formParams = array();
        $_header_accept = ApiClient::selectHeaderAccept(array('application/json'));
        if (!is_null($_header_accept)) {
            $headerParams['Accept'] = $_header_accept;
        }
        $headerParams['Content-Type'] = ApiClient::selectHeaderContentType(array());
  
        
        
        // path params
        
        if ($production_lot_id !== null) {
            $resourcePath = str_replace(
                "{" . "productionLotId" . "}",
                $this->apiClient->getSerializer()->toPathValue($production_lot_id),
                $resourcePath
            );
        }// path params
        
        if ($production_lot_tag !== null) {
            $resourcePath = str_replace(
                "{" . "productionLotTag" . "}",
                $this->apiClient->getSerializer()->toPathValue($production_lot_tag),
                $resourcePath
            );
        }
        // default format to json
        $resourcePath = str_replace("{format}", "json", $resourcePath);

        
        
  
        // for model (json/xml)
        if (isset($_tempBody)) {
            $httpBody = $_tempBody; // $_tempBody is the method argument, if present
        } elseif (count($formParams) > 0) {
            $httpBody = $formParams; // for HTTP post (form)
        }
        
        // this endpoint requires API key authentication
        $apiKey = $this->apiClient->getApiKeyWithPrefix('API-Key');
        if (strlen($apiKey) !== 0) {
            $headerParams['API-Key'] = $apiKey;
        }
        
        
        // make the API Call
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                $resourcePath, 'DELETE',
                $queryParams, $httpBody,
                $headerParams
            );
            
            return array(null, $statusCode, $httpHeader);
            
        } catch (ApiException $e) {
            switch ($e->getCode()) { 
            }
  
            throw $e;
        }
    }
    
    /**
     * getDuplicateProductionLotById
     *
     * Get a duplicated a productionLot by id
     *
     * @param int $production_lot_id Id of the productionLot to be duplicated. (required)
     * @return \Infoplus\Model\ProductionLot
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function getDuplicateProductionLotById($production_lot_id)
    {
        list($response, $statusCode, $httpHeader) = $this->getDuplicateProductionLotByIdWithHttpInfo ($production_lot_id);
        return $response; 
    }


    /**
     * getDuplicateProductionLotByIdWithHttpInfo
     *
     * Get a duplicated a productionLot by id
     *
     * @param int $production_lot_id Id of the productionLot to be duplicated. (required)
     * @return Array of \Infoplus\Model\ProductionLot, HTTP status code, HTTP response headers (array of strings)
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function getDuplicateProductionLotByIdWithHttpInfo($production_lot_id)
    {
        
        // verify the required parameter 'production_lot_id' is set
        if ($production_lot_id === null) {
            throw new \InvalidArgumentException('Missing the required parameter $production_lot_id when calling getDuplicateProductionLotById');
        }
  
        // parse inputs
        $resourcePath = "/beta/productionLot/duplicate/{productionLotId}";
        $httpBody = '';
        $queryParams = array();
        $headerParams = array();
        $formParams = array();
        $_header_accept = ApiClient::selectHeaderAccept(array('application/json'));
        if (!is_null($_header_accept)) {
            $headerParams['Accept'] = $_header_accept;
        }
        $headerParams['Content-Type'] = ApiClient::selectHeaderContentType(array());
  
        
        
        // path params
        
        if ($production_lot_id !== null) {
            $resourcePath = str_replace(
                "{" . "productionLotId" . "}",
                $this->apiClient->getSerializer()->toPathValue($production_lot_id),
                $resourcePath
            );
        }
        // default format to json
        $resourcePath = str_replace("{format}", "json", $resourcePath);

        
        
  
        // for model (json/xml)
        if (isset($_tempBody)) {
            $httpBody = $_tempBody; // $_tempBody is the method argument, if present
        } elseif (count($formParams) > 0) {
            $httpBody = $formParams; // for HTTP post (form)
        }
        
        // this endpoint requires API key authentication
        $apiKey = $this->apiClient->getApiKeyWithPrefix('API-Key');
        if (strlen($apiKey) !== 0) {
            $headerParams['API-Key'] = $apiKey;
        }
        
        
        // make the API Call
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                $resourcePath, 'GET',
                $queryParams, $httpBody,
                $headerParams, '\Infoplus\Model\ProductionLot'
            );
            
            if (!$response) {
                return array(null, $statusCode, $httpHeader);
            }

            return array(\Infoplus\ObjectSerializer::deserialize($response, '\Infoplus\Model\ProductionLot', $httpHeader), $statusCode, $httpHeader);
            
        } catch (ApiException $e) {
            switch ($e->getCode()) { 
            case 200:
                $data = \Infoplus\ObjectSerializer::deserialize($e->getResponseBody(), '\Infoplus\Model\ProductionLot', $e->getResponseHeaders());
                $e->setResponseObject($data);
                break;
            }
  
            throw $e;
        }
    }
    
    /**
     * getProductionLotByFilter
     *
     * Search productionLots by filter
     *
     * @param string $filter Query string, used to filter results. (optional)
     * @param int $page Result page number.  Defaults to 1. (optional)
     * @param int $limit Maximum results per page.  Defaults to 20.  Max allowed value is 250. (optional)
     * @param string $sort Sort results by specified field. (optional)
     * @return \Infoplus\Model\ProductionLot[]
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function getProductionLotByFilter($filter = null, $page = null, $limit = null, $sort = null)
    {
        list($response, $statusCode, $httpHeader) = $this->getProductionLotByFilterWithHttpInfo ($filter, $page, $limit, $sort);
        return $response; 
    }


    /**
     * getProductionLotByFilterWithHttpInfo
     *
     * Search productionLots by filter
     *
     * @param string $filter Query string, used to filter results. (optional)
     * @param int $page Result page number.  Defaults to 1. (optional)
     * @param int $limit Maximum results per page.  Defaults to 20.  Max allowed value is 250. (optional)
     * @param string $sort Sort results by specified field. (optional)
     * @return Array of \Infoplus\Model\ProductionLot[], HTTP status code, HTTP response headers (array of strings)
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function getProductionLotByFilterWithHttpInfo($filter = null, $page = null, $limit = null, $sort = null)
    {
        
  
        // parse inputs
        $resourcePath = "/beta/productionLot/search";
        $httpBody = '';
        $queryParams = array();
        $headerParams = array();
        $formParams = array();
        $_header_accept = ApiClient::selectHeaderAccept(array('application/json'));
        if (!is_null($_header_accept)) {
            $headerParams['Accept'] = $_header_accept;
        }
        $headerParams['Content-Type'] = ApiClient::selectHeaderContentType(array());
  
        // query params
        
        if ($filter !== null) {
            $queryParams['filter'] = $this->apiClient->getSerializer()->toQueryValue($filter);
        }// query params
        
        if ($page !== null) {
            $queryParams['page'] = $this->apiClient->getSerializer()->toQueryValue($page);
        }// query params
        
        if ($limit !== null) {
            $queryParams['limit'] = $this->apiClient->getSerializer()->toQueryValue($limit);
        }// query params
        
        if ($sort !== null) {
            $queryParams['sort'] = $this->apiClient->getSerializer()->toQueryValue($sort);
        }
        
        
        // default format to json
        $resourcePath = str_replace("{format}", "json", $resourcePath);

        
        
  
        // for model (json/xml)
        if (isset($_tempBody)) {
            $httpBody = $_tempBody; // $_tempBody is the method argument, if present
        } elseif (count($formParams) > 0) {
            $httpBody = $formParams; // for HTTP post (form)
        }
        
        // this endpoint requires API key authentication
        $apiKey = $this->apiClient->getApiKeyWithPrefix('API-Key');
        if (strlen($apiKey) !== 0) {
            $headerParams['API-Key'] = $apiKey;
        }
        
        
        // make the API Call
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                $resourcePath, 'GET',
                $queryParams, $httpBody,
                $headerParams, '\Infoplus\Model\ProductionLot[]'
            );
            
            if (!$response) {
                return array(null, $statusCode, $httpHeader);
            }

            return array(\Infoplus\ObjectSerializer::deserialize($response, '\Infoplus\Model\ProductionLot[]', $httpHeader), $statusCode, $httpHeader);
            
        } catch (ApiException $e) {
            switch ($e->getCode()) { 
            case 200:
                $data = \Infoplus\ObjectSerializer::deserialize($e->getResponseBody(), '\Infoplus\Model\ProductionLot[]', $e->getResponseHeaders());
                $e->setResponseObject($data);
                break;
            }
  
            throw $e;
        }
    }
    
    /**
     * getProductionLotById
     *
     * Get a productionLot by id
     *
     * @param int $production_lot_id Id of the productionLot to be returned. (required)
     * @return \Infoplus\Model\ProductionLot
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function getProductionLotById($production_lot_id)
    {
        list($response, $statusCode, $httpHeader) = $this->getProductionLotByIdWithHttpInfo ($production_lot_id);
        return $response; 
    }


    /**
     * getProductionLotByIdWithHttpInfo
     *
     * Get a productionLot by id
     *
     * @param int $production_lot_id Id of the productionLot to be returned. (required)
     * @return Array of \Infoplus\Model\ProductionLot, HTTP status code, HTTP response headers (array of strings)
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function getProductionLotByIdWithHttpInfo($production_lot_id)
    {
        
        // verify the required parameter 'production_lot_id' is set
        if ($production_lot_id === null) {
            throw new \InvalidArgumentException('Missing the required parameter $production_lot_id when calling getProductionLotById');
        }
  
        // parse inputs
        $resourcePath = "/beta/productionLot/{productionLotId}";
        $httpBody = '';
        $queryParams = array();
        $headerParams = array();
        $formParams = array();
        $_header_accept = ApiClient::selectHeaderAccept(array('application/json'));
        if (!is_null($_header_accept)) {
            $headerParams['Accept'] = $_header_accept;
        }
        $headerParams['Content-Type'] = ApiClient::selectHeaderContentType(array());
  
        
        
        // path params
        
        if ($production_lot_id !== null) {
            $resourcePath = str_replace(
                "{" . "productionLotId" . "}",
                $this->apiClient->getSerializer()->toPathValue($production_lot_id),
                $resourcePath
            );
        }
        // default format to json
        $resourcePath = str_replace("{format}", "json", $resourcePath);

        
        
  
        // for model (json/xml)
        if (isset($_tempBody)) {
            $httpBody = $_tempBody; // $_tempBody is the method argument, if present
        } elseif (count($formParams) > 0) {
            $httpBody = $formParams; // for HTTP post (form)
        }
        
        // this endpoint requires API key authentication
        $apiKey = $this->apiClient->getApiKeyWithPrefix('API-Key');
        if (strlen($apiKey) !== 0) {
            $headerParams['API-Key'] = $apiKey;
        }
        
        
        // make the API Call
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                $resourcePath, 'GET',
                $queryParams, $httpBody,
                $headerParams, '\Infoplus\Model\ProductionLot'
            );
            
            if (!$response) {
                return array(null, $statusCode, $httpHeader);
            }

            return array(\Infoplus\ObjectSerializer::deserialize($response, '\Infoplus\Model\ProductionLot', $httpHeader), $statusCode, $httpHeader);
            
        } catch (ApiException $e) {
            switch ($e->getCode()) { 
            case 200:
                $data = \Infoplus\ObjectSerializer::deserialize($e->getResponseBody(), '\Infoplus\Model\ProductionLot', $e->getResponseHeaders());
                $e->setResponseObject($data);
                break;
            }
  
            throw $e;
        }
    }
    
    /**
     * getProductionLotTags
     *
     * Get the tags for a productionLot.
     *
     * @param int $production_lot_id Id of the productionLot to get tags for (required)
     * @return void
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function getProductionLotTags($production_lot_id)
    {
        list($response, $statusCode, $httpHeader) = $this->getProductionLotTagsWithHttpInfo ($production_lot_id);
        return $response; 
    }


    /**
     * getProductionLotTagsWithHttpInfo
     *
     * Get the tags for a productionLot.
     *
     * @param int $production_lot_id Id of the productionLot to get tags for (required)
     * @return Array of null, HTTP status code, HTTP response headers (array of strings)
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function getProductionLotTagsWithHttpInfo($production_lot_id)
    {
        
        // verify the required parameter 'production_lot_id' is set
        if ($production_lot_id === null) {
            throw new \InvalidArgumentException('Missing the required parameter $production_lot_id when calling getProductionLotTags');
        }
  
        // parse inputs
        $resourcePath = "/beta/productionLot/{productionLotId}/tag";
        $httpBody = '';
        $queryParams = array();
        $headerParams = array();
        $formParams = array();
        $_header_accept = ApiClient::selectHeaderAccept(array('application/json'));
        if (!is_null($_header_accept)) {
            $headerParams['Accept'] = $_header_accept;
        }
        $headerParams['Content-Type'] = ApiClient::selectHeaderContentType(array());
  
        
        
        // path params
        
        if ($production_lot_id !== null) {
            $resourcePath = str_replace(
                "{" . "productionLotId" . "}",
                $this->apiClient->getSerializer()->toPathValue($production_lot_id),
                $resourcePath
            );
        }
        // default format to json
        $resourcePath = str_replace("{format}", "json", $resourcePath);

        
        
  
        // for model (json/xml)
        if (isset($_tempBody)) {
            $httpBody = $_tempBody; // $_tempBody is the method argument, if present
        } elseif (count($formParams) > 0) {
            $httpBody = $formParams; // for HTTP post (form)
        }
        
        // this endpoint requires API key authentication
        $apiKey = $this->apiClient->getApiKeyWithPrefix('API-Key');
        if (strlen($apiKey) !== 0) {
            $headerParams['API-Key'] = $apiKey;
        }
        
        
        // make the API Call
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                $resourcePath, 'GET',
                $queryParams, $httpBody,
                $headerParams
            );
            
            return array(null, $statusCode, $httpHeader);
            
        } catch (ApiException $e) {
            switch ($e->getCode()) { 
            }
  
            throw $e;
        }
    }
    
    /**
     * updateProductionLot
     *
     * Update a productionLot
     *
     * @param \Infoplus\Model\ProductionLot $body ProductionLot to be updated. (required)
     * @return void
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function updateProductionLot($body)
    {
        list($response, $statusCode, $httpHeader) = $this->updateProductionLotWithHttpInfo ($body);
        return $response; 
    }


    /**
     * updateProductionLotWithHttpInfo
     *
     * Update a productionLot
     *
     * @param \Infoplus\Model\ProductionLot $body ProductionLot to be updated. (required)
     * @return Array of null, HTTP status code, HTTP response headers (array of strings)
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function updateProductionLotWithHttpInfo($body)
    {
        
        // verify the required parameter 'body' is set
        if ($body === null) {
            throw new \InvalidArgumentException('Missing the required parameter $body when calling updateProductionLot');
        }
  
        // parse inputs
        $resourcePath = "/beta/productionLot";
        $httpBody = '';
        $queryParams = array();
        $headerParams = array();
        $formParams = array();
        $_header_accept = ApiClient::selectHeaderAccept(array('application/json'));
        if (!is_null($_header_accept)) {
            $headerParams['Accept'] = $_header_accept;
        }
        $headerParams['Content-Type'] = ApiClient::selectHeaderContentType(array('application/json'));
  
        
        
        
        // default format to json
        $resourcePath = str_replace("{format}", "json", $resourcePath);

        
        // body params
        $_tempBody = null;
        if (isset($body)) {
            $_tempBody = $body;
        }
  
        // for model (json/xml)
        if (isset($_tempBody)) {
            $httpBody = $_tempBody; // $_tempBody is the method argument, if present
        } elseif (count($formParams) > 0) {
            $httpBody = $formParams; // for HTTP post (form)
        }
        
        // this endpoint requires API key authentication
        $apiKey = $this->apiClient->getApiKeyWithPrefix('API-Key');
        if (strlen($apiKey) !== 0) {
            $headerParams['API-Key'] = $apiKey;
        }
        
        
        // make the API Call
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                $resourcePath, 'PUT',
                $queryParams, $httpBody,
                $headerParams
            );
            
            return array(null, $statusCode, $httpHeader);
            
        } catch (ApiException $e) {
            switch ($e->getCode()) { 
            }
  
            throw $e;
        }
    }
    
    /**
     * updateProductionLotCustomFields
     *
     * Update a productionLot custom fields
     *
     * @param \Infoplus\Model\ProductionLot $body ProductionLot to be updated. (required)
     * @return void
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function updateProductionLotCustomFields($body)
    {
        list($response, $statusCode, $httpHeader) = $this->updateProductionLotCustomFieldsWithHttpInfo ($body);
        return $response; 
    }


    /**
     * updateProductionLotCustomFieldsWithHttpInfo
     *
     * Update a productionLot custom fields
     *
     * @param \Infoplus\Model\ProductionLot $body ProductionLot to be updated. (required)
     * @return Array of null, HTTP status code, HTTP response headers (array of strings)
     * @throws \Infoplus\ApiException on non-2xx response
     */
    public function updateProductionLotCustomFieldsWithHttpInfo($body)
    {
        
        // verify the required parameter 'body' is set
        if ($body === null) {
            throw new \InvalidArgumentException('Missing the required parameter $body when calling updateProductionLotCustomFields');
        }
  
        // parse inputs
        $resourcePath = "/beta/productionLot/customFields";
        $httpBody = '';
        $queryParams = array();
        $headerParams = array();
        $formParams = array();
        $_header_accept = ApiClient::selectHeaderAccept(array('application/json'));
        if (!is_null($_header_accept)) {
            $headerParams['Accept'] = $_header_accept;
        }
        $headerParams['Content-Type'] = ApiClient::selectHeaderContentType(array('application/json'));
  
        
        
        
        // default format to json
        $resourcePath = str_replace("{format}", "json", $resourcePath);

        
        // body params
        $_tempBody = null;
        if (isset($body)) {
            $_tempBody = $body;
        }
  
        // for model (json/xml)
        if (isset($_tempBody)) {
            $httpBody = $_tempBody; // $_tempBody is the method argument, if present
        } elseif (count($formParams) > 0) {
            $httpBody = $formParams; // for HTTP post (form)
        }
        
        // this endpoint requires API key authentication
        $apiKey = $this->apiClient->getApiKeyWithPrefix('API-Key');
        if (strlen($apiKey) !== 0) {
            $headerParams['API-Key'] = $apiKey;
        }
        
        
        // make the API Call
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                $resourcePath, 'PUT',
                $queryParams, $httpBody,
                $headerParams
            );
            
            return array(null, $statusCode, $httpHeader);
            
        } catch (ApiException $e) {
            switch ($e->getCode()) { 
            }
  
            throw $e;
        }
    }
    
}
